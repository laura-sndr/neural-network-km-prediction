...
train_set, test_set = train_test_split(final_df_train, test_size=0.0000001, random_state=42)

train_data = KmClass(final_df_train, p2rank_dir_train)
...
max_folds_to_run = 5  # Limit the number of folds to process

# Placeholder to store fold metrics
fold_results = {"train_loss": [], "val_loss": [], "train_r2": [], "val_r2": [], "train_pearson": [], "val_pearson": []}
all_predictions = []
...
for fold, (train_idx, val_idx) in enumerate(kf.split(train_set)):
    ...
    # Stop processing after max_folds_to_run
    if fold >= max_folds_to_run:
        print("Reached maximum number of folds to run. Stopping cross-validation.")
        break
    ...
    train_df = final_df_train.iloc[train_idx]   
    val_df = final_df_train.iloc[val_idx]      # Create a DataFrame for the validation subset
    
    scale_data = KmClass(train_df, p2rank_dir_train)
    amino_scaler = scale_data.amino_scaler
    descriptor_scaler_robust = scale_data.descriptor_scaler_robust
    descriptor_scaler_minmax = scale_data.descriptor_scaler_minmax
    km_scaler = scale_data.km_scaler

    # Extract min and max values from km_scaler
    km_min = km_scaler.data_min_[0]
    km_max = km_scaler.data_max_[0]

    val_data = KmClass(val_df, p2rank_dir_train, amino_scaler, descriptor_scaler_robust, descriptor_scaler_minmax, km_scaler)
    test_data = KmClass(final_df_test, p2rank_dir_test, amino_scaler, descriptor_scaler_robust, descriptor_scaler_minmax, km_scaler)
    
    # Create DataLoaders
    train_loader = DataLoader(scale_data, batch_size=32, shuffle=True, collate_fn=collate_fn, num_workers=16)
    val_loader = DataLoader(val_data, batch_size=32, shuffle=False, collate_fn=collate_fn, num_workers=16)
    ...
    # Assuming y_test contains the scaled predictions
    ...
    # Save to a TSV file for this fold
    fold_predictions.to_csv(f"/fast/sandner/Thesis/Thesis/Final_data/pickle_files/{folder}/fold_{fold+1}_predictions.tsv", sep='\t', index=False)


    # Store fold results
    fold_results["train_loss"].append(average_loss)
    fold_results["val_loss"].append(average_val_loss)
    fold_results["train_r2"].append(average_r2)
    fold_results["val_r2"].append(average_val_r2)
    fold_results["train_pearson"].append(average_pearson)
    fold_results["val_pearson"].append(average_val_pearson)

# After all folds, save aggregated predictions for comparison
aggregated_predictions = pd.DataFrame({
    'Predicted Value': all_predictions
})

# Save all predictions to a TSV file
aggregated_predictions.to_csv(f"/fast/sandner/Thesis/Thesis/Final_data/pickle_files/{folder}/all_folds_predictions.tsv", sep='\t', index=False)
...
for metric, values in fold_results.items():
    print(f"{metric}: {sum(values) / num_folds:.4f} ± {np.std(values):.4f}")
...
plt.plot(folds, fold_results["train_loss"], label="Training Loss", marker='o', color='blue')
plt.plot(folds, fold_results["val_loss"], label="Validation Loss", marker='o', color='red')
...
plt.legend()
plt.grid()
plt.savefig(f"/fast/sandner/Thesis/Thesis/Final_data/Graphs/model_output/{folder}/foldwise_loss_{version}.png")
plt.close()

# r2
plt.figure(figsize=(10, 5))
plt.plot(folds, fold_results["train_r2"], label="Training R²", marker='o', color='blue')
plt.plot(folds, fold_results["val_r2"], label="Validation R²", marker='o', color='red')
plt.xticks(range(0, 6, 1))
plt.ylim(top=1)
plt.xlabel("Fold")
plt.ylabel("R² Score")
...
plt.legend()
plt.grid()
plt.savefig(f"/fast/sandner/Thesis/Thesis/Final_data/Graphs/model_output/{folder}/foldwise_r2_{version}.png")
plt.close()

# pearson
plt.figure(figsize=(10, 5))
plt.plot(folds, fold_results["train_pearson"], label="Training Pearson's R", marker='o', color='blue')
plt.plot(folds, fold_results["val_pearson"], label="Validation Pearson's R", marker='o', color='red')
plt.xticks(range(0, 6, 1))
plt.ylim(top=1)
plt.xlabel("Fold")
plt.ylabel("Pearson's R Score")
plt.title("Pearson's R Across Folds")
plt.legend()
plt.grid()
plt.savefig(f"/fast/sandner/Thesis/Thesis/Final_data/Graphs/model_output/{folder}/foldwise_pearson_{version}.png")
plt.close()

metrics = ["train_loss", "val_loss", "train_r2", "val_r2", "train_pearson", "val_pearson"]
averages = [np.mean(fold_results[m]) for m in metrics]
std_devs = [np.std(fold_results[m]) for m in metrics]

plt.figure(figsize=(10, 5))
x = np.arange(len(metrics))

plt.bar(x, averages, yerr=std_devs, capsize=5, color=["blue", "red", "green", "orange"])
plt.xticks(x, ["Train Loss", "Val Loss", "Train R²", "Val R²", "Train Pearson's R", "Val Pearson's R"])
plt.ylabel("Metric Value")
plt.title("Average Metrics Across Folds")
plt.grid(axis='y')
plt.savefig(f"/fast/sandner/Thesis/Thesis/Final_data/Graphs/model_output/{folder}/summarized_metrics_{version}.png")
plt.close()
...
metrics = {
    "Metric": ["Train Loss", "Validation Loss", "Train R²", "Validation R²", "Train Pearson's R", "Validation Pearson's R"],
    "Mean": [
        np.mean(fold_results["train_loss"]),
        np.mean(fold_results["val_loss"]),
        np.mean(fold_results["train_r2"]),
        np.mean(fold_results["val_r2"]),
        np.mean(fold_results["train_pearson"]),
        np.mean(fold_results["val_pearson"]),
    ],
    "Std": [
        np.std(fold_results["train_loss"]),
        np.std(fold_results["val_loss"]),
        np.std(fold_results["train_r2"]),
        np.std(fold_results["val_r2"]),
        np.std(fold_results["train_pearson"]),
        np.std(fold_results["val_pearson"]),
    ],
}
...
# Save to CSV
results_cv.to_csv(f"/fast/sandner/Thesis/Thesis/Final_data/pickle_files/{folder}/results_cv_{version}.csv", index=False)

print("Cross-validation summary saved to 'cross_validation_summary.csv'")
...
with open(f"/fast/sandner/Thesis/Thesis/Final_data/pickle_files/{folder}/metrics_{version}.pkl", "wb") as f:
    pickle.dump({"loss train": loss_values, "r2 train": r2_values, "loss val": loss_values_val, "r2 val": r2_values_val, "pearson train": pearson_values, "pearson val": pearson_values_val}, f)
...
# Plot R2 over epoch
plt.plot(pearson_values, label='Training Pearson R')
plt.plot(pearson_values_val, label='Validation Pearson R')
plt.title('Training and Validation Pearson R')
plt.xlabel('Epochs')
plt.xticks(epochs, [str(epoch + 1) for epoch in epochs])
plt.ylabel('Pearson R')
plt.ylim(top=1)
plt.legend()
plt.savefig(f"/fast/sandner/Thesis/Thesis/Final_data/Graphs/model_output/{folder}/pearson_over_epoch_{version}.png")
plt.close()
