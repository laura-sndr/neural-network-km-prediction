...
for epoch in range(epochs):
    ...
    for i, data in enumerate(train_loader):
        ...
        l2_loss = sum(param.norm(2) for param in net.parameters())

        loss += lambda_l1 * l1_loss + lambda_l2 * l2_loss  # Add L1 and L2 penalty to the loss
...
test_data_df.to_csv("test_data_with_y.csv", index=False)

print("Data saved to test_data_with_y.csv")
...
def log_experiment(version, hyperparameters, epochs, train_r2, val_r2, test_r2):
    ...
    dropout_values = [hyperparameters["dropout1"]]
    weight_decay = [lambda_l2, lambda_l1]
...
first_layer = net.layers[0]  # Index 0 corresponds to the first nn.Linear layer

# Get the weights of the first layer
weights = first_layer.weight.data  # Shape: (hidden_dim1, input_size)

# Calculate importance for each feature
feature_importance = weights.abs().sum(dim=0)  # Sum absolute weights across all neurons in the first layer

# Normalize the feature importance for interpretation
normalized_importance = feature_importance / feature_importance.sum()

# Convert to NumPy for further analysis or visualization
normalized_importance = normalized_importance.cpu().numpy()

# Define feature groups and their colors
protein_end = 2048
descriptor_end = protein_end + 196
fingerprint_end = descriptor_end + 2048
input_size = 4292

colors = []
for i in range(input_size):
    if i < protein_end:
        colors.append('blue')  # Protein information
    elif i < descriptor_end:
        colors.append('green')  # Descriptors
    else:
        colors.append('orange')  # Molecular fingerprints

# Visualize as bar graph
plt.figure(figsize=(10, 6))
plt.bar(range(len(normalized_importance)), normalized_importance, color=colors)
plt.xlabel("Feature Index", fontsize=14)
plt.ylabel("Normalized Importance", fontsize=14)
plt.title("Feature Importance Based on First Layer Weights", fontsize=16)

# Add a legend to distinguish the groups
handles = [
    plt.Rectangle((0, 0), 1, 1, color='blue', label='Protein Information'),
    plt.Rectangle((0, 0), 1, 1, color='green', label='Descriptors'),
    plt.Rectangle((0, 0), 1, 1, color='orange', label='Molecular Fingerprints'),
]
plt.legend(handles=handles, loc='upper right')

plt.savefig(f"/work/sandner/Thesis/Thesis/Final_data/Graphs/model_output/{folder}/feature_importance_{version}.png")
plt.show()


# Feature importance for the first 200 features (100 amino acids and 100 binding site features)
protein_importance_200 = normalized_importance[:200]  # First 200 features

# Define distinct colors for amino acids and binding info
colors_200 = ['blue' if i % 2 == 0 else 'orange' for i in range(len(protein_importance_200))]

# Visualize as bar graph
plt.figure(figsize=(12, 6))
plt.bar(range(len(protein_importance_200)), protein_importance_200, color=colors_200, width=0.8)
plt.xlabel("Feature Index (Amino Acid and Binding Site Features Alternating)", fontsize=14)
plt.ylabel("Normalized Importance", fontsize=14)
plt.title("Feature Importance for First 100 Amino Acids and Respective Binding Site Features", fontsize=16)

# Add a legend to distinguish the groups
handles = [
    plt.Rectangle((0, 0), 1, 1, color='blue', label='Amino Acid Information'),
    plt.Rectangle((0, 0), 1, 1, color='orange', label='Binding Site Information'),
]
plt.legend(handles=handles, loc='upper right')

# Save the graph
plt.savefig(f"/work/sandner/Thesis/Thesis/Final_data/Graphs/model_output/{folder}/feature_importance_protein_200_{version}.png")
plt.close()

# Feature importance for the first 500 features (250 amino acids and 250 binding site features)
protein_importance_500 = normalized_importance[:500]  # First 500 features

# Define distinct colors for amino acids and binding info
colors_500 = ['blue' if i % 2 == 0 else 'orange' for i in range(len(protein_importance_200))]

# Visualize as bar graph
plt.figure(figsize=(12, 6))
plt.bar(range(len(protein_importance_500)), protein_importance_500, color=colors_500, width=0.8)
plt.xlabel("Feature Index (Amino Acid and Binding Site Features Alternating)", fontsize=14)
plt.ylabel("Normalized Importance", fontsize=14)
plt.title("Feature Importance for First 250 Amino Acids and Respective Binding Site Features", fontsize=16)
...
# Save the graph
plt.savefig(f"/work/sandner/Thesis/Thesis/Final_data/Graphs/model_output/{folder}/feature_importance_protein_500_{version}.png")
